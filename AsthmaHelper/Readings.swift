...
class Readings : NSObject {
        ...
        let query = HKAnchoredObjectQuery(type: healthType, predicate: nil, anchor: queryAnchor, limit: 500, resultsHandler: {(query, newSamples,                                               deletedSamples, newAnchor, error) -> Void in
            ...
            dispatch_async(dispatch_get_main_queue()) {
                ...
                for s in samples {
                    ...
                    let r = Reading(date: s.startDate, location: location!, reading: reading, id: s.UUID)
                    r.source = s.sourceRevision.source
                    self.list.append(r)
                }
    ...
    func createReading(r: Reading, successHandler: () -> Void, failHandler: (NSError?) -> Void) {
    ...
    func deleteReading(id: NSUUID, successHandler: () -> Void, failHandler: (NSError?) -> Void) {
